...
from utils import DEFAULT_MARKET_DATA_ID
...
class IBClient(EClient):
    ...
    def get_historical_data(self, request_id, contract, duration, bar_size):
        self.reqHistoricalData(reqId= request_id, contract=contract, endDateTime="", 
                               durationStr=duration, barSizeSetting=bar_size, whatToShow="MIDPOINT", 
    ...
    def get_multiple_historical_data(self, request_id, contracts, duration, 
                                     bar_size, cols_to_use = "close"):
        dfs = []
        for contract in contracts:
            df = self.get_historical_data(contract, duration, bar_size)
            dfs.append(df)
            request_id += 1
        return (pd.concat(dfs).reset_index()
                .pivot(index="time", columns="symbol", values=cols_to_use)
        )
    
    def get_market_data(self, request_id, contract, tick_type=4):
        self.reqMktData(reqId=request_id, contract=contract, genericTickList="", 
    ...
    def get_streaming_returns(self, request_id, interval, pnl_type):
        ...
        for snapshot in self.get_streaming_pnl(request_id=request_id,interval=interval,pnl_type=pnl_type):
